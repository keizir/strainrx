...
W.pages.business.BusinessPartnerships = Class.extend({
    init: function init(options) {
        ...
        this.fetchGrowers();
        this.ui.$locations.on('change', this.fetchGrowers.bind(this));
        this.ui.$addBtn.on('click', this.onAddButtonClick.bind(this));

        this.initDispensaryLookup();
    },

    fetchGrowers: function onGrowerChange() {
        var that = this;

        return $.ajax({
            method: 'GET',
            url: '/api/v1/businesses/{0}/locations/{1}/partnerships/'.format(that.businessId, that.ui.$locations.val()),
    ...
    onAddButtonClick: function onAddButtonClick() {
        ...
        that.createPartnership(that.newPartner).success(function() {
            that.fetchGrowers().success(function() {
    ...
    onPartnershipDelete: function onPartnershipDelete(e) {
        ...
        }).success(that.fetchGrowers.bind(that));
    },

    initDispensaryLookup: function initDispensaryLookup() {
        var that = this;
        var DispensaryLookup = W.pages.dispensary.DispensaryLookup;

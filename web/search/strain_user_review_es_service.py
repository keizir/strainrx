...
class StrainUserReviewESService(BaseElasticService):
    ...
    def save_strain_review(self, updated_review, db_strain_id, db_user_id):
        ...
        if len(es_reviews) > 0:
            ...
            review["removed_date"] = updated_review.removed_date

            url = '{base}{index}/{type}/{es_id}'.format(base=self.BASE_ELASTIC_URL, index=self.URLS.get('USER_REVIEWS'),
                                                        type=es_mappings.TYPES.get('strain_user_review'),
                                                        es_id=review_es_id)
            es_response = self._request(self.METHODS.get('PUT'), url, data=json.dumps(review))
        else:
            url = '{base}{index}/{type}'.format(base=self.BASE_ELASTIC_URL, index=self.URLS.get('USER_REVIEWS'),
                                                type=es_mappings.TYPES.get('strain_user_review'))
            es_response = self._request(self.METHODS.get('POST'), url, data=json.dumps({
                ...
                "removed_date": updated_review.removed_date
            }))

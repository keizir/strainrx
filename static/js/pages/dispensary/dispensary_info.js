...
W.pages.dispensary.DispensaryInfo = Class.extend({
    ...
    showHeader: function showHeader() {
        ...
        this.clickVerifiedBadge();
        this.hideVerifiedBadge();
    ...
    clickVerifiedBadge: function clickVerifiedBadge() {
        $('.badge-image').on('click', function (e) {
            var parentOffset = $(this).parent().offset();
            var relX = e.pageX - parentOffset.left;
            var relY = e.pageY - parentOffset.top;
            var x = e.pageX - $('.location-name-row .location-name').offset().left;
            var y = e.pageY - $('.location-name-row .location-name').offset().top;
            console.log(x, y);

            $('.badge-popup').css('top', y)
            $('.badge-popup').css('left', x)
            $('.badge-popup').show()
        });
    },

    hideVerifiedBadge: function hideVerifiedBadge() {
        $(document).mouseup(function(e) {
            var badge_icon = $(".badge-image");
            var badge_popup = $('.badge-popup');
            if((!badge_icon.is(e.target) && badge_icon.has(e.target).length == 0) && !badge_popup.is(e.target) && badge_popup.has(e.target).length == 0) {
                badge_popup.hide();
            }
        });
    },
